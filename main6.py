...
openai.api_key = api_key  #  Use openai.api_key for the classic OpenAI library.

client = openai.OpenAI(api_key=api_key) # Keeping the client for consistency.
...
class graph2text_client:
    ...
    def __init__(self, model_path, max_length: int = 128):
        ...
        if not os.path.exists(model_path):
            ...
            raise ValueError(f"FastToG model path not found: {model_path}")
    ...
    def generate(self, triples: str, max_length=None) -> str:
        ...
        inputs = self.tokenizer(TASK_PREFIX + triples, return_tensors="pt", padding=True, truncation=True).to(self.device)
...
class KnowledgeGraph:
    ...
    def __init__(self, fasttog_model_path="YOUR_FASTTOG_MODEL_PATH"):
        ...
        try:
            ...
            self.g2t_client = graph2text_client(model_path=fasttog_model_path)  # Replace
        ...
        except FileNotFoundError as e: #Added FileNotFoundError

            logger.error(f"FastToG model not found: {e}")
    ...
    def bfs_summarize(self, start_label: str, max_hops: int = 3, max_neighbors: int = 5) -> str:
        ...
        triples_str = " ".join(triples)  # join the triples
...
# SECTION B: LLM Multi-Step Reasoning (with error handling and DSPy integration)

# --------------------------------------------


lm = dspy.OpenAI(model="gpt-4o", api_key=api_key, max_tokens=200)
...
class GenerateSolution(dspy.Signature):
    ...
    """Generates a solution to the given prompt."""
...
class ValidateSolution(dspy.Signature):

    """Validates the given solution."""

    solution = dspy.InputField()

    validation = dspy.OutputField()


class AssessSolution(dspy.Signature):

    """Assesses the solution and validation, returning a score and confidence."""

    solution = dspy.InputField()

    validation = dspy.InputField()

    score = dspy.OutputField(desc="float between 0 and 1")

    confidence = dspy.OutputField(desc="float between 0 and 1")


class EvaluateSolution(dspy.Signature):

    """give a final evaluation of if the answer is correct"""
...
def llm_generate_solution(prompt: str) -> str:
    ...
    try:
        ...
        pred = dspy.Predict(GenerateSolution)(prompt=prompt)

        return pred.solution

    except Exception as e:

        logger.error(f"DSPy Predict (generate_solution) error: {e}")
...
def llm_validate(solution: str) -> str:
    ...
    try:
        ...
        pred = dspy.Predict(ValidateSolution)(solution=solution)

        return pred.validation

    except Exception as e:

        logger.error(f"DSPy Predict (validate) error: {e}")
...
def llm_assess(solution: str, validation_text: str) -> Tuple[float, float]:
    ...
    try:
        ...
        pred = dspy.Predict(AssessSolution)(solution=solution, validation=validation_text)
        ...
        logger.warning(f"DSPy or Parsing LLM assess output failed: {e}; using random values.")
...
def llm_evaluate(solution: str) -> bool:
    ...
    try:
        ...
        pred = dspy.Predict(EvaluateSolution)(solution=solution)

        return "correct" in pred.evaluation.lower()

    except Exception as e:

        logger.error(f"DSPy Predict (evaluate) error: {e}")

        return False  # Changed from returning "DSPY_ERROR" to False
...
class Agent:
    ...
    def run_llm_calls(self):
        ...
        if self.solution.startswith("DSPY"):

            return

        self.validation = llm_validate(self.solution)

        if self.validation.startswith("DSPY"):
    ...
    def build_child_prompt(self, kg: KnowledgeGraph) -> str:
        ...
        for entity in entities:
            ...
            if extra_info and "No specific graph info" not in extra_info:  #Added condition
...
class MASTERFastToG:
    ...
    def __init__(self, question: str, entity: str, max_expansion: int, num_branches: int, fasttog_model_path="YOUR_FASTTOG_MODEL_PATH"):

        self.kg = KnowledgeGraph(fasttog_model_path=fasttog_model_path)  # Initialize the Knowledge Graph
...
if __name__ == "__main__":
    ...
    fasttog_model_path = "YOUR_FASTTOG_MODEL_PATH" #  Replace with your model


    try:

        system = MASTERFastToG(question, entity, max_expansion, num_branches, fasttog_model_path)
        ...
        print(f"Error: FastToG model not found at path: {fasttog_model_path}.  Please update 'fasttog_model_path'.")
